(function () {
    ...
    var ChatWidgetConfig = (function () {
      var defaults = {
        ...
        uiText: {
          header: "Jogin AI",
          welcome: "Cześć, jestem Jogin AI. Jak mogę Ci pomóc?",
    ...
    var css = `
      ...
      .category-card{
        border:1px solid var(--chat-border); border-radius:var(--radius-sm); padding:12px; background:#fff; color:var(--chat-text);
        box-shadow:0 2px 6px rgba(0,0,0,.06); display:flex; flex-direction:column; gap:8px;
      }
      .category-title{font-weight:600; font-size:15px; line-height:1.3}
      .category-buttons{display:flex; flex-wrap:wrap; gap:6px}
      .category-button{
        padding:6px 12px; border:none; background:var(--chat-secondary); color:#fff;
        border-radius:var(--radius-xs); text-decoration:none; font-size:13px; font-weight:600; text-align:center; display:inline-block;
      }
      .order-card{
        border:1px solid var(--chat-border); border-radius:var(--radius-sm); padding:12px; background:#fff; color:var(--chat-text);
      ...
      .order-header strong { color: #333; font-size: 15px; }
      .order-header .order-status {
        background-color: #e8f6ee; color: #007b00; font-weight: 600;
      ...
      .timeline h3 { font-size: 14px; margin-bottom: 6px; color: #333; }
      .timeline table {
        width: 100%; border-collapse: collapse; background: #fafafa;
        border-radius: 8px; overflow: hidden;
      }
      .timeline th {
        background-color: #f3f3f3; text-align: left; font-weight: 600;
        font-size: 13px; padding: 8px; border-bottom: 1px solid #e0e0e0;
      }
      .timeline td { padding: 8px; border-bottom: 1px solid #eee; font-size: 13px; color: #333; }
      ...
      container.innerHTML = `
        <button id="chat-widget-button" aria-label="${ChatWidgetConfig.uiText.btnOpenAria}">
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 4h16a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H10l-6 4v-4H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z" stroke="white" stroke-width="2" stroke-linejoin="round"/>
    ...
    function hasProducts(json) {
      return json && Array.isArray(json.products) && json.products.length > 0;
    }
    function hasDetails(json) {
      return json && Array.isArray(json.details) && json.details.length > 0;
    }
    function hasOrderInfo(json) {
      return json && json.type === "order_status" && Array.isArray(json.order_info) && json.order_info.length > 0;
    }

    function renderCategoryCards(container, details) {
      details.forEach(function (d) {
        var card = document.createElement("div");
        card.className = "category-card";
        var html = '';
        if (d.name) html += '<div class="category-title">' + d.name + '</div>';
        if (d.link && !d.buttons) {
          html += '<a href="' + d.link + '" target="_blank" rel="noopener noreferrer" class="product-button">' + ChatWidgetConfig.uiText.btnProduct + '</a>';
        }
        if (Array.isArray(d.buttons) && d.buttons.length > 0) {
          html += '<div class="category-buttons">';
          d.buttons.forEach(function (btn) {
            html += '<a href="' + (btn.url || btn.link || '#') + '" target="_blank" rel="noopener noreferrer" class="category-button">' + (btn.label || btn.text || ChatWidgetConfig.uiText.btnProduct) + '</a>';
          });
          html += '</div>';
        }
        card.innerHTML = html;
      ...
      products.forEach(function (p) {
        ...
        if (Array.isArray(p.buttons) && p.buttons.length > 0) {
          p.buttons.forEach(function (btn) {
            btnHTML += '<a href="' + (btn.url || btn.link || '#') + '" target="_blank" rel="noopener noreferrer" class="product-button">' + (btn.label || btn.text || ChatWidgetConfig.uiText.btnProduct) + '</a>';
          });
        } else if (p.link) {
          btnHTML = '<a href="' + p.link + '" target="_blank" rel="noopener noreferrer" class="product-button">' + ChatWidgetConfig.uiText.btnProduct + '</a>';
        }
        card.innerHTML =
          '<div class="product-top">' +
            (p.image ? '<img src="' + p.image + '" alt="' + (p.title || '') + '" class="product-image" onerror="this.style.display=\'none\'" />' : '') +
    ...
    function renderOrderCards(container, orders) {
      orders.forEach(function (p) {
        ...
        if (Array.isArray(p.timeline) && p.timeline.length > 0) {
          timelineHTML =
            '<div class="timeline"><h3>Historia zamówienia:</h3><table><thead><tr><th>Etap</th><th>Data</th></tr></thead><tbody>' +
            p.timeline.map(function(item) {
              return '<tr><td>' + (item.label || '') + '</td><td>' + new Date(item.at).toLocaleString('pl-PL') + '</td></tr>';
            }).join('') +
            '</tbody></table></div>';
        }
        ...
        card.innerHTML =
          ...
          '<div class="order-info">' +
            (p.created_at ? '<div><strong>Data utworzenia:</strong> ' + new Date(p.created_at).toLocaleString('pl-PL') + '</div>' : '') +
            (contact.name ? '<div><strong>Kontakt:</strong> ' + contact.name + '</div>' : '') +
            (contact.email ? '<div><strong>E-mail:</strong> ' + contact.email + '</div>' : '') +
            (contact.phone ? '<div><strong>Telefon:</strong> ' + contact.phone + '</div>' : '') +
          '</div>' +
          timelineHTML;
        container.appendChild(card);
      });
      ...
      function sendMessage() {
        ...
        .then(function (data) {
          ...
          try {
            ...
            if (match) {
              jsonBlock = JSON.parse(match[1].trim());
            }
          } catch (e) {
            console.warn("JSON parse z bloku markdown nie powiodł się:", e);
          }
          ...
          if (!jsonBlock) {
            try {
              ...
              if (trimmed.charAt(0) === '{' || trimmed.charAt(0) === '[') {
                jsonBlock = JSON.parse(trimmed);
              }
            } catch (e) {}
          }

          var descriptionText = rawText
          ...
          if (jsonBlock && hasOrderInfo(jsonBlock)) {
            if (descriptionText) {
            ...
            renderOrderCards(botContainer, jsonBlock.order_info);
            rendered = true;
          }

          if (!rendered && jsonBlock && (hasProducts(jsonBlock) || hasDetails(jsonBlock))) {
            if (descriptionText) {
              var desc2 = document.createElement("div");
              desc2.innerHTML = markdownToHTML(descriptionText);
              desc2.style.marginBottom = "8px";
              botContainer.appendChild(desc2);
            }
            if (hasDetails(jsonBlock)) {
              renderCategoryCards(botContainer, jsonBlock.details);
            }
            if (hasProducts(jsonBlock)) {
              renderProductCards(botContainer, jsonBlock.products);
            }
            rendered = true;
          }
